...
public class CompositeContext {
	...
	private final Map<RecordingListener<CompositeContext>, CompositeContext> contexts = new HashMap<>();

	CompositeContext(RecordingListener<CompositeContext>... listeners) {
		this(Arrays.asList(listeners));
	}

	CompositeContext(List<? extends RecordingListener<CompositeContext>> listeners) {
		// Could be a .stream().collect(toMap(...)) but toMap fails on null values:
		// https://bugs.openjdk.java.net/browse/JDK-8148463
		for (RecordingListener<CompositeContext> listener : listeners) {
			this.contexts.put(listener, listener.createContext());
		}
	}

	CompositeContext byListener(RecordingListener<CompositeContext> listener) {
		return this.contexts.get(listener);
	}
